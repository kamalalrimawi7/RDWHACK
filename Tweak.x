...
- (void)viewDidLoad {
    ...
    img.layer.borderWidth = 2; img.layer.borderColor = RDW_GOLD.CGColor;
    ...
    self.msgL.textColor = [UIColor systemRedColor];
    self.msgL.textAlignment = NSTextAlignmentCenter; self.msgL.font = [UIFont systemFontOfSize:12];
    [self.box addSubview:self.msgL];
    self.fullNameField = [[UITextField alloc] initWithFrame:CGRectMake(20, 220, w-40, 50)];
    self.fullNameField.placeholder = @"أدخل اسمك الرباعي هنا";
    self.fullNameField.backgroundColor = [UIColor colorWithWhite:1 alpha:0.1];
    ...
    self.inputField.placeholder = @"أدخل كود التفعيل";
...
#pragma clang diagnostic pop
    ...
    [btn1 setTitleColor:[UIColor whiteColor] forState:0]; btn1.layer.cornerRadius = 15;
    ...
    btn2.frame = CGRectMake(20, 420, w-40, 50);
    ...
    dev.textColor = [UIColor darkGrayColor]; dev.textAlignment = NSTextAlignmentCenter; dev.font = [UIFont systemFontOfSize:10];
...
- (void)goCheck {
    ...
    if (code.length < 4) { [self showError:@"الكود غير صالح!"];
        return;
    }
...
- (void)syncUserWithDays:(int)days name:(NSString *)name code:(NSString *)code completion:(void(^)(BOOL ok))completion {
    ...
    [[[NSURLSession sharedSession] dataTaskWithURL:[NSURL URLWithString:userPath] completionHandler:^(NSData *d, NSURLResponse *r, NSError *e) {
        ...
        [pReq setHTTPBody:[NSJSONSerialization dataWithJSONObject:update options:0 error:nil]];
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.codeIn.placeholder = @"أدخل كود التمديد هنا";
        self.codeIn.backgroundColor = [UIColor colorWithWhite:1 alpha:0.1];
...
- (void)doStack {
    ...
    if (c.length < 4) {
        ...
        self.codeIn.placeholder = @"يرجى ادخال رمز صالح"; return;
    }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (j && ![j isEqual:[NSNull null]]) {
                ...
                if ([uBy isEqualToString:myU]) {
                    ...
                    self.codeIn.text = @"";
                    self.codeIn.placeholder = @"لقد استخدمت هذا الكود سابقا!";
                } else if ([uBy isEqualToString:@""]) {
                     [self syncStack:c];
                } else {
                     [RDWCore playSoundNamed:@"error"];
                     self.codeIn.placeholder = @"هذا الكود مستخدم من جهاز آخر!"; self.codeIn.text = @"";
                }
            } else { [RDWCore playSoundNamed:@"error"];
                self.codeIn.placeholder = @"كود غير صحيح!"; self.codeIn.text = @""; }
        });
    }] resume];
}
...
- (void)addDays:(int)days code:(NSString *)c {
    ...
    [[[NSURLSession sharedSession] dataTaskWithURL:[NSURL URLWithString:uPath] completionHandler:^(NSData *d, NSURLResponse *r, NSError *e) {
        ...
        [re setHTTPMethod:@"PATCH"];
        [re setHTTPBody:[NSJSONSerialization dataWithJSONObject:up options:0 error:nil]];
...
%new - (void)rdw_handleP:(UILongPressGestureRecognizer *)g {
    if (g.state == UIGestureRecognizerStateBegan) {
        ...
        p.alpha = 0;
        p.transform = CGAffineTransformMakeScale(0.6, 0.6); [self addSubview:p];
        [UIView animateWithDuration:0.4 delay:0 usingSpringWithDamping:0.7 initialSpringVelocity:0.6 options:0 animations:^{ p.alpha=1;
            p.transform=CGAffineTransformIdentity; } completion:nil];
    }
}
